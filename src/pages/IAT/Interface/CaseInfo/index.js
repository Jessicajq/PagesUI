  ...
  queryCaseDataWithLoading = caseId => {
    ...
    dispatch({
      ...
      if (caseData) {
        this.setState({
          caseData,
          postShellChecked: caseData && caseData.postShell !== null,
          preShellChecked: caseData && caseData.preShell !== null,
        });
      }
    });
  };
      ...
      <Skeleton loading={loading}>
        <Form {...formItemLayout}>
          ...
          <Form.Item label="Header设置" className={styles.keyValueContainer}>
            {caseData.headerValues &&
              caseData.headerValues.map(item => {
                if (item.type === 'add') {
                  return (
                    <Button
                      key={item.type}
          ...
          <Form.Item
            label="请求参数"
            className={getFieldValue('paramType') !== 4 ? styles.keyValueContainer : ''}
          >
            {getFieldValue('paramType') !== 4 &&
              caseData.paramsValues &&
              caseData.paramsValues.map(item => {
                if (item.type === 'add') {
                  return (
                    <Button
                      key={item.type}
              ...
              <Form.Item className={styles.keyValueContainer}>
                {getFieldValue('assertType') === 1 ? (
                  <div style={{ marginTop: 10 }}>
                    {caseData.textAssertValues &&
                      caseData.textAssertValues.map(item => {
                        if (item.type === 'add') {
                          return (
                            <Button
                              key={item.type}
                        ...
                        return (
                          ...
                          >
                            <TextArea
                              ...
                              onBlur={e =>
                                this.handleKeyValueChange(item.id, { keyValue: e.target.value })
                              }
                            />
                            <div className={styles.section_delete}>
                              <Icon
                                type="minus-circle"
                                onClick={() => this.handleDeleteValue(item.id)}
                              />
                            </div>
                          </div>
                        );
                  ...
                  caseData.jsonAssertValues &&
                  caseData.jsonAssertValues.map(item => {
                    if (item.type === 'add') {
                      return (
                        <Button
                          key={item.type}
            ...
            <div>
              {getFieldDecorator('extractType', {
                ...
                <Radio.Group>
                  ...
                  <Radio value={2}>正则提取</Radio>
                </Radio.Group>,
              )}
              <Form.Item className={styles.keyValueContainer}>
                {getFieldValue('extractType') !== 0 &&
                  caseData.jsonExtractValues &&
                  caseData.jsonExtractValues.map(item => {
                    if (item.type === 'add') {
                      return (
                        <Button
                          key={item.type}
