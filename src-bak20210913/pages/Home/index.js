...
export default class Home extends PureComponent {
  ...
  queryHomeData = () => {
    ...
    dispatch({
      ...
      if (home.homeData) {
        const formatResult = [];
        home.homeData.mounthTask &&
          home.homeData.mounthTask.forEach(item => {
            formatResult.push({
              x: item.dayTime * 1000,
              y1: item.total,
              y2: item.sucess,
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            {record.taskType === 1 && (
              <a href={`#/task/api/immediate/detail?${record.id}`}>{text}</a>
            )}
            {record.taskType === 2 && <a href={`#/task/api/timing/detail?${record.id}`}>{text}</a>}
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            {record.status === 3 && record.taskType === 1 && (
              <a href={`#/task/api/immediate/report?${record.id}`}>报告</a>
            )}
            {record.status === 3 && record.taskType === 2 && (
              <a href={`#/task/api/timing/report?${record.id}`}>报告</a>
            )}
          </div>
        ),
      },
