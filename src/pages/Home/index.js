  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            {(record.taskType === 1) && (<a href={`#/task/api/immediate/detail?${record.id}`}>{text}</a>)}
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            {(record.status === 3 && record.taskType === 1) && (<a href={`#/task/api/immediate/report?${record.id}`}>报告</a>)}
            {(record.status === 3 && record.taskType === 2) && (<a href={`#/task/api/timing/report?${record.id}`}>报告</a>)}
          </div>
        ),
      },
    ];
