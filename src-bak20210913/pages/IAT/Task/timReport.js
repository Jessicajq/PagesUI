...
import { Scatter, Pie, Curved, CurvedTwo } from '@/components/Charts';
  ...
  queryTaskInfo = id => {
    this.props
      ...
      .then(() => {
        ...
        const charData = [];
        iatTask.taskResult.result &&
          iatTask.taskResult.result.forEach(item => {
            charData.push({
              title: item.label,
  ...
  render() {
    ...
    const content = (
      <div className={styles.pageHeaderContent}>
        <p>开始时间：{moment(parseInt(taskResult.startTime)).format('YYYY-MM-DD HH:mm:ss')}</p>
        <p>结束时间：{moment(parseInt(taskResult.endTime)).format('YYYY-MM-DD HH:mm:ss')}</p>
        <p>
          用例统计：<span className={styles.total}>{taskResult.total}</span> 总数
      ...
      <PageHeaderWrapper title={taskResult.testname} content={content} extraContent={extraContent}>
        <Row gutter={24}>
          <Col xl={12} lg={24} md={24} sm={24} xs={24}>
            <Card title="本次任务结果" bordered={false} loading={loading}>
              <Collapse className={styles.resultCollapse}>
                {taskResult.result &&
                  taskResult.result.map((item, index) => (
                    <Panel
                      header={
                        <a target="_blank" rel="noopener noreferrer" href={`/case/api?${item.id}`}>
                          {item.label}
                        </a>
                      }
                      key={index}
          ...
          <Col xl={12} lg={24} md={24} sm={24} xs={24}>
            <Card title="每日耗时数据对比" bordered={false} loading={loading}>
